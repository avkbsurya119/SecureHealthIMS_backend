...
### Test 9: grantConsent › should create new consent if none exists
...
| **Record Created** | true |
| **Status** | granted |
| **Consent ID** | Generated by database |
| **Timestamp** | Current timestamp (granted_at) |

**Validations:**

- Verified upsert logic creates new record when needed
- Confirmed INSERT operation performed
- Validated new consent record has status 'granted'
- Ensured timestamp recorded for audit trail
- Tested first-time consent grant scenario
- Confirmed audit trail creation

### Test 10: grantConsent › should update existing consent

**Status: PASS** | Duration: 1ms

**Test Inputs:**

| **Patient ID** | patient-existing |
| --- | --- |
| **Consent Type** | treatment |
| **Existing Record** | { id: 'consent-123', patient_id: 'patient-existing', consent_type: 'treatment', status: 'denied', denied_at: '2024-01-01T00:00:00Z' } |
| **Action** | Update status to 'granted' |

**Expected Outputs:**

| **SQL Operation** | UPDATE |
| --- | --- |
| **Consent ID** | consent-123 (preserved) |
| **New Status** | granted |
| **Updated Timestamp** | granted_at set to current time |
| **Return Value** | Updated consent object |

**Validations:**

- Verified upsert logic updates existing record
- Confirmed UPDATE operation preserves consent ID
